...
(function attachCncModules(root, factory) {
  ...
  function ensureState(state) {
    ...
    const hadRawInventory = isRecord(state.inventory) && isRecord(state.inventory.rawMaterial);
    const legacyMaterial = nonNegativeNumber(state.material) ? state.material : 0;
    const legacyCapacity = nonNegativeNumber(state.capacity) ? state.capacity : null;
    const savedRawCapacity = hadRawInventory && isRecord(state.inventory.capacities) &&
      nonNegativeNumber(state.inventory.capacities.raw)
      ? state.inventory.capacities.raw
      : null;
    const inventory = ensureInventory(state);
    const finance = ensureFinance(state);
    if (!hadRawInventory && legacyMaterial > 0) {
      // Older game saves only know one generic material balance. Keep it as
      // "legacy" stock instead of guessing which material type it represents.
      inventory.rawMaterial.legacy = roundStock(legacyMaterial);
    }
    const preferredRawCapacity = hadRawInventory
      ? (savedRawCapacity ?? legacyCapacity ?? DEFAULT_CAPACITIES.raw)
      : (legacyCapacity ?? DEFAULT_CAPACITIES.raw);
    inventory.capacities.raw = roundStock(Math.max(preferredRawCapacity, currentUsage(inventory, 'raw')));
    if (Object.hasOwn(state, 'material')) state.material = currentUsage(inventory, 'raw');
    if (Object.hasOwn(state, 'capacity')) state.capacity = inventory.capacities.raw;
  ...
  function book(state, category, amount, description, meta = {}, aggregateKey = null) {
    if (!isRecord(state) || !Number.isFinite(state.money)) return { ok: false, code: 'invalid_balance' };
    if (!Number.isFinite(amount) || amount === 0) return { ok: false, code: 'invalid_amount' };
    const normalizedCategory = normalizeCategory(category);
    if (!normalizedCategory) return { ok: false, code: 'invalid_category' };
    if (typeof description !== 'string' || !description.trim()) return { ok: false, code: 'invalid_description' };
    ...
    if (aggregateKey !== null && aggregateKey !== undefined) {
      if (typeof aggregateKey !== 'string' || !aggregateKey.trim() || aggregateKey.length > 160) {
        return { ok: false, code: 'invalid_aggregate_key' };
      }
      const key = aggregateKey.trim();
      const finance = ensureFinance(state);
      if (!finance) return { ok: false, code: 'invalid_state' };
      for (let index = finance.transactions.length - 1; index >= 0; index -= 1) {
        const existing = finance.transactions[index];
        if (isRecord(existing) && existing.category === normalizedCategory &&
            existing.meta?.aggregateKey === key && Number.isFinite(existing.amount)) {
          existing.amount = roundStock(existing.amount + amount);
          state.money = roundStock(state.money + amount);
          return {
            ok: true,
            aggregated: true,
            balance: state.money,
            transaction: { ...existing, meta: { ...(existing.meta || {}) } }
          };
        }
      }
      clonedMeta.value.aggregateKey = key;
    }

    const result = record(state, normalizedCategory, amount, description, clonedMeta.value);
    if (!result.ok) return result;
    state.money = roundStock(state.money + amount);
    return { ...result, aggregated: false, balance: state.money };
  }
