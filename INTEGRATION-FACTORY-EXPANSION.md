...
| 2 | 6 | hall-level-2.webp | 1920 × 1200, 16:10 | 3 columns × 2 rows |
| 3 | 8 | hall-level-3.webp | 1920 × 1200, 16:10 | 4 columns × 2 rows |
...
hall-level-2.webp and hall-level-3.webp are intentionally named in the module but do not exist yet. Create both at exactly 1920 × 1200 pixels (16:10) as empty hall backgrounds. The backgrounds may include architecture, floor markings, walkways and fixed building details. They must not contain CNC machines, machine silhouettes, machine labels, machine status, purchase UI or any purchased-machine artwork. Machine images remain independent overlays rendered by the game per bay.

The level 2 artwork should provide three clearly separated bays in each of two rows. The level 3 artwork should provide four clearly separated bays in each of two rows. Keep clear foreground space inside every slot for machine sprite overlays and labels. Until these finished assets are committed, level 2/3 are module layout placeholders only; do not point the live hall UI at missing assets.
